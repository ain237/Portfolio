...
## [View the Jupyter notebook on this website](https://ain237.github.io/portfolio-website/xG_model_notebook/ " ")
...
The first run with only x,y coordinates show the model to be 89.42% accurate. This seems sufficiently good. To understand whether the model understands the game mechanics, we will generate a heatmap in top of an image of a soccer field that shows the spots with high probability for scoring.
...
After introducing the other variables to our algorithm, the model’s accuracy score stays the same at 89,42%. To further understand the model, we will calculate a confusion matrix, that intends to further explain, which predictions are accurate.


![Figure 3]({{"assets/xg-post/cfmatrix.png" | relative_url }}  "Figure 3: Confusion matrix")

The confusion matrix shows that when the model makes a 1 or 0 prediction about whether a goal will be scored, it is surprisingly inaccurate. When the model predicts that a goal will be scored, the predictions are right only on about 2.2% of times. When the model however predicts that no goal will be scored, it is incredibly accurate. How should this be explained? The principle on how the model works is that it sees whether the shot is shot from the area where it's likely that the goal will be shot. The predictions also take account any shots shot during the game, which may not be anywhere near the goal. In a football game overwhelming majority shots are missed. Therefore the model is not useful for predicting individual shots, but because it's high average accuracy, it's useful for evaluating team performace with large datasets.

For our xG model we are the most interested in the ability of our model to predict probabilities of scoring. The heamap in Figure 2 shows that our model accurately understands the game-mechanics, and its accuracy score after introducing new variables is 89.42%. The model is therefore considered reliable. 
